...
\textbf{GRPO for Factual Calibration:} Conversely, the SFT $\rightarrow$ GRPO pipeline leveraged the \texttt{accuracy\_reward\_func} to triple the factual Answer Accuracy (15.0\% vs 5.0\% for SFT). Because the reward heavily penalizes hallucinations, GRPO learned a highly calibrated, cautious policy. Answering is deemed ``risky,'' causing the model to clarify more often to avoid penalties (increasing Clarify Ratio to 1.37 and pulling Action F1 down to 34.5\%). This demonstrates that online RL with deterministic rewards is highly effective at enforcing factual logic and safety, while DPO is better suited for tone and formatting alignment.
