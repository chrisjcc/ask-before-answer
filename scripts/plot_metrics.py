...
def extract_metrics(state_path: str):
    ...
    return {"train_loss": train_loss, "eval_loss": eval_loss, "log_history": log_history}
...
def plot_comparisons(results: dict):
    ...
    for model_name, metrics in results.items():
        if metrics and metrics["eval_loss"]:
            ...
            plt.plot(steps, losses, label=f"{model_name} Eval Loss", linewidth=2, linestyle="--")
...
def plot_grpo_specifics(grpo_metrics: dict):
    ...
    for key in keys_found:
        data = [(entry["step"], entry[key]) for entry in log_history if key in entry and "step" in entry]
        if not data:
            continue
...
def main():
    ...
    for model_name, model_dir in MODEL_DIRS.items():
        ...
        if not os.path.exists(state_path):
            ...
            checkpoints = [d for d in os.listdir(model_dir) if d.startswith("checkpoint-")] if os.path.exists(model_dir) else []
            if checkpoints:
                checkpoints.sort(key=lambda x: int(x.split("-")[-1]))
                state_path = os.path.join(model_dir, checkpoints[-1], "trainer_state.json")
