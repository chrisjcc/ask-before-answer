...
This dataset contains the processed Supervised Fine-Tuning (SFT) and Direct Preference Optimization (DPO) data for the AskBeforeAnswer clarification model.
...
This model has been fine-tuned to detect ambiguity and ask clarification questions before answering.
It contains both the Supervised Fine-Tuning (SFT) and Direct Preference Optimization (DPO) artifacts.
...
def main():
    ...
    release_body = os.environ.get("RELEASE_BODY", "Automated deployment via GitHub Actions.")
    ...
    if os.path.exists("data/sft_data.jsonl"):
        api.upload_file(path_or_fileobj="data/sft_data.jsonl", path_in_repo="sft_data.jsonl", repo_id=DATASET_REPO, repo_type="dataset")
    if os.path.exists("data/dpo_data.jsonl"):
        api.upload_file(path_or_fileobj="data/dpo_data.jsonl", path_in_repo="dpo_data.jsonl", repo_id=DATASET_REPO, repo_type="dataset")
    
    with tempfile.NamedTemporaryFile("w", delete=False) as f:
        f.write(generate_dataset_readme(release_tag, release_name, release_body))
        ds_readme_path = f.name
    api.upload_file(path_or_fileobj=ds_readme_path, path_in_repo="README.md", repo_id=DATASET_REPO, repo_type="dataset")
    ...
    if os.path.exists("models/sft_output/final"):
        ...
        api.upload_folder(folder_path="models/sft_output/final", path_in_repo="sft_final", repo_id=MODEL_REPO, repo_type="model")
    
    if os.path.exists("models/dpo_output/final"):
        print("Uploading DPO final model...")
        api.upload_folder(folder_path="models/dpo_output/final", path_in_repo="dpo_final", repo_id=MODEL_REPO, repo_type="model")
    
    with tempfile.NamedTemporaryFile("w", delete=False) as f:
        f.write(generate_model_readme(release_tag, release_name, release_body))
        mdl_readme_path = f.name
    api.upload_file(path_or_fileobj=mdl_readme_path, path_in_repo="README.md", repo_id=MODEL_REPO, repo_type="model")
