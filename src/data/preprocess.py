...
class SyntheticGenerator:
    ...
    SYSTEM_PROMPT = """You are a clarification-seeking question-understanding agent trained on AmbigQA.
...
Reasoning: <detailed reasoning explaining why the question has multiple interpretations, or why it is perfectly clear>
        ...
        rows.append(
            {
                ...
                "neg_response": (single_ans_flat[0] if is_ambiguous and single_ans_flat else "Could you please clarify the specific details?"),
            }
        )
    ...
    if synthetic_cfg and synthetic_cfg.get("enabled", False):
        ...
        if synthetic_cfg.get("filter_bad_rows", False):
            if "is_bad_row" in df.columns:
                ...
                df = df[df["is_bad_row"] != True].reset_index(drop=True)
        ...
        if synthetic_cfg.get("balance_classes", False):
            ...
            if min_len > 0:
                ...
                df = pd.concat([clarify_df, answer_df]).sample(frac=1, random_state=42).reset_index(drop=True)
...
def prepare_dpo_dataset(df: pd.DataFrame, output_path: str, synthetic_cfg: Optional[DictConfig] = None) -> None:
    ...
    for _, row in df.iterrows():
        ...
        generate_hard_negatives = synthetic_cfg and synthetic_cfg.get("generate_hard_negatives", False)
        
        if generate_hard_negatives:
            neg_resp = clean_response(row.get("neg_response", "Could you please clarify?"))
            if action == "Clarify":
