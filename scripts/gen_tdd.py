...
story.append(P("AskBeforeAnswer &mdash; System &amp; Pipeline Architecture", "DocSubtitle"))
...
story.append(data_table(
    ["Layer", "Role"],
    [
        ["Flash Attention (flash-attn)", "Computes attention without materializing the full N&times;N matrix, reducing memory from O(N^2) to O(N). Targets Ampere+ GPUs (RTX A6000, 3090, 4090)."],
...
story.append(data_table(
    ...
    [
        ...
        ["facet_logic_reward_func", "Enforces Clarify &rarr; non-empty Facets, and Answer &rarr; empty Facets"],
...
story.append(data_table(
    ["Config Path", "Governs"],
    [
        ["conf/config.yaml", "Root config &mdash; imports all sub-configs via the defaults list"],
...
story.append(data_table(
    ...
    [
        ...
        ["Bayesian sweeps", "scripts/run_sweep_trial.py is the W&amp;B sweep agent entry point; it fetches hyperparameters, patches the local Hydra config, and runs dvc exp run"],
        ["Sweep reporting", "scripts/generate_sweep_report.py pulls cloud metrics grouped by Sweep ID, plots Validation Curves, and statelessly regenerates docs/sweep_report.md"],
        ["Model registry", "publish-model-artifact creates/verifies an immutable W&amp;B artifact and checks the production alias + registry digest before any Hugging Face push is permitted"],
        ["Weave evaluation", "scripts/evaluate.py runs the dual-scoring pipeline (LLM-as-a-judge + ActionScorer) through Weave, producing a dynamic leaderboard"],
...
story.append(data_table(
    ...
    [
        ...
        ["make run-pipeline", "Full DVC pipeline: data &rarr; SFT &rarr; SFT eval &rarr; DPO &rarr; SFT+DPO eval"],
        ["make train TRAIN_VARIANT=&lt;v&gt; / make train-sft / -dpo / -orpo / -grpo", "Run a specific training variant"],
        ["make sweep FINE_TUNE_METHOD=&lt;m&gt; COUNT=&lt;n&gt; / make sweep-sft / -dpo / -grpo", "Initialize and run a W&amp;B Bayesian sweep"],
...
story.append(data_table(
    ...
    [
        ["Production GPU serving", "1x NVIDIA T4 / L4 / RTX 3060 (8&ndash;12GB VRAM)", "4-bit / 8-bit quantized (bitsandbytes)"],
...
story.append(P("END OF DOCUMENT | TDD-ASKBEFOREANSWER-001 v1.0.0 | INTERNAL / ENGINEERING", "Footer"))

build_doc(os.path.join(OUTPUT_DIR, "TDD_AskBeforeAnswer_SystemArchitecture.pdf"), RUNNING_TITLE, FOOTER_LEFT, story)
print("done")
