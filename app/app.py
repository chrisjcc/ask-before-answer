...
st.markdown("""
This application demonstrates **clarification-seeking behavior** in Large Language Models. 
When given an ambiguous question, instead of hallucinating or assuming an intent, the model detects the ambiguity, explains the reasoning, identifies missing facets, and asks a targeted clarification question.
""")
    ...
    if st.button("Generate Response") and user_query:
        with st.spinner("Analyzing..."):
            ...
            if "Action: Clarify" in response:
                st.success("Model successfully detected ambiguity and requested clarification!")
            elif "Action: Answer" in response:
                st.success("Model determined the question was unambiguous and answered directly.")
