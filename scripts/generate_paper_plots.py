...
def plot_grpo_metrics(grpo_hist, out_dir):
    ...
    # Total Train Reward
    if "train/reward" in grpo_hist.columns:
        plt.figure(figsize=(8, 5))
        df = grpo_hist.dropna(subset=["train/reward"])
        plt.plot(df["_step"], df["train/reward"], label="Total Reward", color="blue")
        plt.title("GRPO Total Training Reward")
        plt.xlabel("Training Step")
        plt.ylabel("Reward")
        plt.legend()
        plt.tight_layout()
        out_path = os.path.join(out_dir, "grpo_total_reward.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        print(f"Saved {out_path}")
        plt.close()

    # Reward Components Convergence
    reward_cols = [c for c in grpo_hist.columns if "reward" in c and "mean" in c]
    if reward_cols:
        ...
        plt.tight_layout()
        out_path = os.path.join(out_dir, "grpo_reward_components.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        print(f"Saved {out_path}")
        plt.close()
