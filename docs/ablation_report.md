...
3. **DPO-First Approach:** Since `dpo_only` performed relatively well without relying on SFT, we should experiment with expanding the preference dataset and applying DPO directly to the `base` model to see if it can align the model beyond its zero-shot capabilities.
