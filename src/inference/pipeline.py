...
class ClarifyOrActPipeline:
    ...
    def __init__(self, model_path: str, is_peft: bool = True) -> None:
        ...
        if self.device == "cpu":
            logger.warning("vLLM requires a GPU! CPU fallback is not supported. This will crash.")
    ...
    def batch_generate(self, questions: List[str], system_prompt: Optional[str] = None) -> List[str]:
