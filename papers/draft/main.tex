...
We benchmark our hybrid pipelines against standard end-to-end models utilizing the \texttt{unsloth/qwen2.5-7b-instruct-unsloth-bnb-4bit} base architecture, evaluated on the validation split of the \texttt{sewon/ambig\_qa} dataset. Our primary comparisons are between the classic $\text{SFT} \rightarrow \text{DPO}$ pipeline and our proposed $\text{SFT} \rightarrow \text{GRPO}$ approach.
...
\item \textbf{LLM-as-a-Judge Quality Metrics:} Ambiguity Detection F1, Clarification Quality F1, and Clarification Usefulness, scored subjectively by a strong teacher model (\texttt{google/gemma-2-2b-it}).
\end{enumerate}
    ...
    \begin{small}
      \begin{sc}
        \begin{tabular}{lccccccc}
          ...
          SFT & \textbf{62.0\%} & 72.5\% & 38.7\% & 55.6\% & 1.30 & 5.0\% & \textbf{100\%} \\
          SFT $\rightarrow$ DPO & \textbf{62.0\%} & 70.8\% & \textbf{45.7\%} & \textbf{58.2\%} & \textbf{1.17} & 0.0\% & \textbf{100\%} \\
          SFT $\rightarrow$ GRPO & \textbf{62.0\%} & \textbf{73.2\%} & 34.5\% & 53.9\% & 1.37 & \textbf{15.0\%} & \textbf{100\%} \\
          \bottomrule
        \end{tabular}
      \end{sc}
    \end{small}
...
\textbf{DPO Induces Reward Hacking:} The SFT $\rightarrow$ DPO pipeline reveals a critical vulnerability of offline preference optimization when applied to multi-objective reasoning tasks. While DPO successfully mitigated over-clarification (dropping the Clarify Ratio to a near-ideal 1.17 and boosting Action F1 to 45.7\%), it suffered from complete semantic collapse on generative tasks (0\% Answer Accuracy). Because ambiguity resolution requires a delicate balance of caution and helpfulness, static preference pairs force the model to over-index on stylistic formatting. The model effectively ``games'' the preference margins to perfect the structural classification task, sacrificing all factual generative grounding.

\textbf{GRPO for Dynamic Calibration:} To solve this offline reward hacking, the SFT $\rightarrow$ GRPO pipeline leverages online reinforcement learning with the \texttt{accuracy\_reward\_func}. By replacing static preference pairs with dynamic programmatic rewards, GRPO allows the model to explore the Pareto frontier between structural compliance and generative accuracy. This successfully tripled the factual Answer Accuracy (15.0\% vs 5.0\% for SFT) without sacrificing schema adherence. Because the reward heavily penalizes hallucinations, GRPO learned a highly calibrated, cautious policy. Answering is deemed ``risky,'' causing the model to clarify more often to avoid penalties (increasing Clarify Ratio to 1.37 and pulling Action F1 down to 34.5\%). This demonstrates that online RL with deterministic rewards is highly effective at enforcing factual logic and safety, bypassing the reward hacking inherent to DPO.
...
Running DPO directly on the base model (\texttt{dpo\_only}) results in catastrophic structural collapse, yielding an abysmal Macro F1 of 50.7\% and a Facet Generation Rate of 10.9\%. This mathematically demonstrates that preference optimization is strictly a stylistic polisher; it is incapable of teaching structural schema or complex logic from scratch and requires a strong behavioral prior (SFT).

\textbf{The Merits of Single-Stage ORPO:}
As a single-stage algorithm, ORPO performs admirably without a dedicated SFT warm-up, achieving a 95.6\% Facet Generation Rate and a 49.3\% Macro F1. While it is highly efficient, it underperforms the dedicated two-stage SFT $\rightarrow$ DPO pipeline (100\% FGR, 58.2\% Macro F1), suggesting that for highly complex structural tasks, explicitly separating behavioral cloning from preference alignment yields more robust results.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{figures/val_curve_lr.png}
\caption{Validation loss across different learning rates during hyper-parameter tuning.}
\label{fig:val_curve_lr}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/dpo_margin.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/dpo_implicit_rewards.png}

\vspace{0.3cm}
\includegraphics[width=0.48\textwidth]{figures/dpo_logprobs.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/dpo_accuracies.png}
\caption{Detailed DPO training metrics: Reward Margins (top left), Implicit Rewards (top right), Log Probabilities (bottom left), and Accuracies (bottom right).}
\label{fig:dpo_details}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/grpo_total_reward.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/kl_divergence.png}

\vspace{0.3cm}
\includegraphics[width=0.48\textwidth]{figures/grpo_reward_components.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/reward_convergence.png}
\caption{Detailed GRPO training metrics: Total Training Reward (top left), KL Divergence from the reference policy (top right), individual programmatic reward components (bottom left), and overall reward convergence (bottom right).}
...
To ensure full reproducibility, we report the exact hyperparameters utilized during the Supervised Fine-Tuning (SFT), Direct Preference Optimization (DPO), and Group Relative Policy Optimization (GRPO) stages. To mitigate the extreme VRAM overhead inherent to online reinforcement learning and preference optimization, all training stages utilized QLoRA (Quantized Low-Rank Adaptation). Specifically, the base models were loaded in 4-bit precision via the \texttt{bitsandbytes} library (\texttt{bnb-4bit}), while adapter weights were injected into the attention and MLP modules and trained in \texttt{bfloat16} precision. 

All experiments were conducted using the \texttt{transformers} and \texttt{trl} libraries, heavily utilizing the Unsloth framework for efficient hardware acceleration via custom Triton kernels and Flash Attention. This combination of 4-bit quantization and Unsloth's optimized kernels enabled the successful execution of memory-intensive GRPO pipelines on single NVIDIA RTX A6000 and A100 GPUs.
    ...
    \begin{small}
      \begin{sc}
        \begin{tabular}{lcccccc}
          ...
          \midrule
          Ambiguity Detection & 0.966 & 0.968 & \textbf{0.970} & 0.946 & 0.968 & \textbf{0.970} \\
          Clarification Quality & 0.784 & 0.786 & 0.796 & 0.778 & 0.796 & \textbf{0.800} \\
          Usefulness & 0.880 & 0.882 & \textbf{0.896} & 0.878 & \textbf{0.896} & 0.892 \\
          Model Accuracy & 0.620 & \textbf{0.640} & 0.620 & 0.620 & 0.620 & 0.620 \\
          Clarify Precision & 0.617 & 0.630 & 0.641 & \textbf{0.657} & 0.622 & 0.634 \\
          Clarify Recall & \textbf{0.967} & \textbf{0.967} & 0.833 & 0.767 & 0.933 & 0.867 \\
          Clarify F1 & 0.753 & \textbf{0.763} & 0.725 & 0.708 & 0.747 & 0.732 \\
          Action F1 (Answer) & 0.174 & 0.250 & 0.387 & \textbf{0.457} & 0.240 & 0.345 \\
          Macro F1 & 0.464 & 0.507 & 0.556 & \textbf{0.582} & 0.493 & 0.539 \\
          Answer Accuracy & 0.050 & 0.050 & 0.050 & 0.000 & 0.050 & \textbf{0.150} \\
          Facet Generation Rate & 0.085 & 0.109 & \textbf{1.000} & \textbf{1.000} & 0.956 & \textbf{1.000} \\
          Clarify Ratio & 1.567 & 1.533 & 1.300 & \textbf{1.167} & 1.500 & 1.370 \\
          \bottomrule
        \end{tabular}
      \end{sc}
    \end{small}
...
\section{Appendix: Hyper-parameter Optimization (GRPO)}
\label{sec:appendix_hyperparams}
Given the sensitivity of Reinforcement Learning algorithms to hyper-parameters, we conducted a rigorous Bayesian hyper-parameter sweep for the GRPO stage utilizing Weights \& Biases. The objective of the sweep was to maximize the mean accuracy reward (\texttt{train/rewards/accuracy\_reward/mean}). 

To ensure computational efficiency, the sweep employed a Hyperband early stopping protocol (\texttt{min\_iter: 50}, \texttt{eta: 3}), which dynamically terminates under-performing trials early in the optimization process. We searched over the following parameter space:
\begin{itemize}
    \item \textbf{KL Penalty ($\beta$)}: Uniform distribution over $[0.01, 0.2]$
    \item \textbf{Learning Rate}: Log-uniform distribution over $[1\times10^{-6}, 1\times10^{-4}]$
\end{itemize}

The top 5 performing configurations are detailed in Table \ref{tab:grpo_sweep}. The winning trial (\texttt{u8xjwcjv}) converged on a highly aggressive learning rate with a minimal KL penalty, yielding the highest objective score. This configuration was selected for the final SFT $\rightarrow$ GRPO pipeline deployed in our main results.

\begin{table*}[!ht]
  \caption{Top Performing GRPO Hyper-parameter Configurations}
  \label{tab:grpo_sweep}
  \begin{center}
    \begin{small}
      \begin{tabular}{llccc}
        \toprule
        Rank & Run ID & $\beta$ Penalty & Learning Rate & Mean Accuracy Reward \\
        \midrule
        \textbf{1} & \textbf{u8xjwcjv} & \textbf{0.0101} & \textbf{3.34e-05} & \textbf{0.2479} \\
        2 & 496fdb4q & 0.0585 & 1.65e-05 & 0.2325 \\
        3 & f4m7i3qn & 0.1645 & 2.95e-06 & 0.2321 \\
        4 & c4ygyajl & 0.1572 & 1.13e-06 & 0.2046 \\
        5 & rhwp8er0 & 0.1095 & 1.16e-06 & 0.1383 \\
        \bottomrule
      \end{tabular}
    \end{small}
  \end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/sweep_val_curve_dppeb7ks.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/sweep_val_curve_h74t7aj3.png}
\caption{Validation curves across hyper-parameter trials demonstrating the Hyperband early stopping protocol, highlighting trials that were dynamically terminated early (e.g., Trial \texttt{dppeb7ks} and \texttt{h74t7aj3}).}
\label{fig:sweep_val_curves}
\end{figure}
